 ...
 */
export function createBot(strategy: string, botsManager: BotsManager): TradingStrategy {
...
export class TradingStrategy {
  ...
  protected async analyzeAndTrade(
    ...
    await this.placeOrder(bot.bot_id, randomCompany.company_id, isBuy, price_in_cents, quantity)
  }
...
export class MomentumStrategy extends TradingStrategy {
  protected async analyzeAndTrade(
    ...
    for (const company of companies) {
      ...
      const closePrices = priceHistory.map((p) => p.close_price_in_cents).reverse()
      ...
      const holding = shareholdings.find((h) => h.company_id === company.company_id)
...
export class DividendStrategy extends TradingStrategy {
  protected async analyzeAndTrade(
    ...
    for (const company of companies) {
      ...
      else if (holding && currentPrice > holding.average_purchase_price_in_cents * 1.2) {
