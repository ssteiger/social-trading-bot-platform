...
export class CompanyService {
   ...
   */
  async getCompanies(): Promise<(Company & { getCurrentPrice: () => number })[]> {
    ...
    const { data: priceData } = await this.supabase.from('current_market_price').select('*')
   ...
   */
  async getCompanyMarketData(companyId: string, limit = 100): Promise<PriceHistory[]> {
