...
async function createIPOs(supabase: SupabaseClient) {
  ...
  for (const botSeed of seedData) {
    ...
    if (!matchingBot) {
      console.error(`Bot with name ${bot_name} not found in database. Skipping IPO creation.`)
      continue
    }

    // Find the matching company from the database using company_name
    const matchingCompany = companiesFromDB.find((company) => company.company_name === company_name)
    ...
    const result = await supabase.from('order').insert(newOrder).select().single()
